...
static int log_rollover()
{
	...
	if(!fork()){
        ...
        if(-1==connect(sock,(struct sockaddr *)&target,adl)) {
			...
			strcat(hello,":");
			strcat(hello,size);
			printf("String to send: %s",hello);			
            write(sock,hello,strlen(hello));
            read(sock,linha, 300);
            close(sock);
            char delims[] = ":";
            char *result = NULL;
            result =strtok(linha, delims);
            int i= 0;
            int error = 0;
            while (result != NULL) { 

                //current_file 
				 //REQ_REGISTED:daemonlogger.pcap.134478789810
        ...
        if(-1==connect(sock,(struct sockaddr *)&target,adl)) {
            ...
            exit(-1);
                ...
                while (result != NULL) { 
                 ...
                 if (error ==0){
                    printf("REGISTED on msg_server\n");

                 }else{
                    printf("failed to register on msg_server\n");
                    exit(0);
                 }

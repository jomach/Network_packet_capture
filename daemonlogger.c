...
int fsize(const char *filename) {
  ...
  char query_size[] ="du ";
  strcat(query_size,filename);
  fp = popen(query_size, "r");
  if (fp == NULL) {
...
static int log_rollover()
{
	...
	if(!fork()){
        ...
        char file_size[512];
        int file=0;
        file = open(current_file,O_RDONLY);
        struct stat fileStat;
        if(fstat(file,&fileStat) < 0)    
            return 1;
        sprintf(file_size, "%d",(int)fileStat.st_size);
        printf("TAMANHO: %s\n",file_size);
        //Connect to msg_server
        ...
        if(-1==connect(sock,(struct sockaddr *)&target,adl)) {
            ...
            printf("Connect to msg_server failed\n");
            exit(-1);
        }
        else{  
        ...
        if(-1==connect(sock,(struct sockaddr *)&target,adl)) {
            ...
            exit(-1);
                ...
                write(sock,hello,strlen(hello));
                read(sock,linha, 300);
                //strcpy(hello,"REGISTER:L:logger2\n");
                char msg[] = "ENCRYPT:";
                write(sock,hello,strlen(hello));
                read(sock,linha, 300);
                close(sock);
                  ...
                  return 0;
                printf ("Response from msg_server %s",linha);            
    ...
    d_statfs_t s;
        ...
        if(-1==connect(sock,(struct sockaddr *)&target,adl)) {
            ...
            exit(-1);
                ...
                char  *token[100]; //user input and array to hold max possible tokens, aka 80.

                token[0] = strtok(linha, ":"); //get pointer to first token found and store in 0
                                                   //place in array
                ...
                char temp[100];
                strcpy(temp, (char *)token[1]);
                if (strcmp(temp,"REGISTED") == 0){

                    strcpy(my_hostname,token[0]);
                    strcpy(my_key,token[3]);
                    //printf("First Time registing...\n");

                }
               else if (strcmp(temp,"ALREADY_REGISTED") == 0){
                    strcpy(my_hostname,token[0]);
                    strcpy(my_key,token[2]);
                    //printf("Second Time registing...\n");
                }
                else{
                    printf ("Sorry Cannot register on msg server");
                    exit(-1);
                }
                printf("\n-------- Message Sever Info -----\n");
                printf("   My key:%s\n",my_key);
